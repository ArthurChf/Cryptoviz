<template>
    <div>
        <input class="h-8 leading-none p-2 bg-inherit outline-none w-40 hover:bg-slate-900 transition duration-[200ms] text-sm" :placeholder="selectedItem.name" v-model="search" @input="updateSearchValue" />
        <Transition :name="TransitionEnum.FADE">
            <div v-if="search.length" class="select-none absolute top-8 left-0 w-40 flex flex-col rounded-b-md flex-nowrap overflow-x-auto scrollbar bg-slate-950 overflow-y-auto scrollbar max-h-32 text-sm">
